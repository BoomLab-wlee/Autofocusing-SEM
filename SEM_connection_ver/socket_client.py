...
class Socket_Client():
    ...
    def load_eval_net(self):
        try:
            self.AENet = nn.DataParallel(evaluator_model.AENet(), device_ids=[0])
            self.AENet.to(self.device)
            self.AENet.eval()
            model_name = 'Trained_network/EvalNet_201201_MAG_240x320.pth'
            checkpoint = torch.load(model_name, map_location='cuda:0')
            self.AENet.load_state_dict(checkpoint['State_dict'])
